...
- Generated by KMS using the KMS and `GenerateDataKey` operation.
- Used to encrypt data larger than 4KB in size.
- Linked to a specific KMS so KMS can tell that a specific DEK was
generated with a specific KMS.
...
- Ciphertext Version - Encrypted version of the DEK.
  - This is encrypted by the KMS that generated it.
  - In the future it can be decrypted by KMS using the KMS assuming
...
- Customer Master Keys (KMS) are isolated to a region.
  - Never leave the region or KMS.
  - Cannot extract a KMS.
...
- Customer managed KMS keys
  ...
  - Can allow other AWS accounts access to KMS keys
...
KMS can create an alias which is a shortcut to a particular KMS.
...
potentially at a different KMS.
...
S3 works with KMS to create an AWS managed KMS. This is the default key
which gets used in the future.

Every time an object is uploaded, S3 uses a dedicated key to encrypt that object
and that key is a data encryption key which KMS generates using the KMS.
The KMS does not need to be managed by AWS and can be a customer managed KMS.
...
When uploading an object, you can create and use a customer managed KMS. This
...
access to the KMS that was used to generate the unique key that encrypted them.
The KMS is used to decrypt the data encryption key for that object.
...
This can be the EBS default (KMS) which is referred to as `aws/ebs` or it
could be a customer managed KMS which you manage yourself.

That key is used by EBS when an encrypted volume is created. The KMS
...
- AWS accounts can be set to encrypt EBS volumes by default.
  - It will use the default KMS unless a different one is chosen.
